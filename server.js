...
const server = http.createServer((req, res) => {
  ...
  } else if (pathname === '/api/day') {
    res.end(JSON.stringify({ day: getDayNumber() }));
  } else if (pathname === '/api/status') {
    const day = getDayNumber();
    const phase = roadmap.find(p => {
      const days = p.weeks.flatMap(w => w.days_data.map(d => d.d));
      return day >= Math.min(...days) && day <= Math.max(...days);
    });
    res.end(JSON.stringify({ day, phase: phase ? { id: phase.id, title: phase.title } : null }));
  } else {
    res.writeHead(404);
...
server.listen(PORT, () => {
  ...
  console.log('  GET /api/roadmap - Full roadmap data');
  console.log('  GET /api/schedule - Daily schedule');
  console.log('  GET /api/day - Current day number');
  console.log('  GET /api/status - Current day and phase');
});
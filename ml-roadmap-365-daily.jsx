  ...
  },
  {
    ...
    weeks: [
      ...
      { id:"2-4", range:"Week 15–16", days:"Days 99–112", title:"Feature Engineering + Kaggle", days_data:[
        ...
        {d:100,t:"Day 100 Milestone!","desc":"Feature Scaling: StandardScaler (z-score), MinMaxScaler, RobustScaler. Which models need scaling?"},
...
export default function MLRoadmap() {
  ...
  return (
    <div style={{fontFamily:"var(--font-sans)", color:"var(--color-text-primary)", maxWidth:640, margin:"0 auto", padding:"0.75rem 0"}}>

      {/* Header */}
      <div style={{marginBottom:"1rem"}}>
        <div style={{display:"flex", alignItems:"center", gap:10, flexWrap:"wrap"}}>
          <h1 style={{fontSize:20, fontWeight:500, margin:0}}>ML Engineer — 365 Days</h1>
          <span style={{fontSize:12, background:"var(--color-background-secondary)", border:"0.5px solid var(--color-border-tertiary)", borderRadius:20, padding:"2px 10px", color:"var(--color-text-secondary)"}}>Day {Math.min(dayNum,365)} of 365</span>
        </div>
        <p style={{fontSize:13, color:"var(--color-text-secondary)", margin:"4px 0 0"}}>Python → ML → Deep Learning → LLM Fine-tuning → MLOps → Ship 3 Products</p>
      </div>

      {/* View toggle */}
      <div style={{display:"flex", gap:6, marginBottom:"1rem", borderBottom:"0.5px solid var(--color-border-tertiary)", paddingBottom:"0.75rem"}}>
        {["roadmap","schedule"].map(v=>(
          <button key={v} onClick={()=>setView(v)} style={{padding:"5px 14px", borderRadius:20, fontSize:13, cursor:"pointer", border:`0.5px solid ${view===v?"var(--color-border-primary)":"var(--color-border-tertiary)"}`, background:view===v?"var(--color-background-secondary)":"transparent", color:view===v?"var(--color-text-primary)":"var(--color-text-secondary)", fontWeight:view===v?500:400, textTransform:"capitalize"}}>
            {v}
          </button>
        ))}
      </div>
      ...
      {view === "schedule" && (
        <div>
          <p style={{fontSize:13, color:"var(--color-text-secondary)", margin:"0 0 12px"}}>Daily commitment: <strong>3h 30min</strong>. Non-negotiable. This is the engine.</p>
          {schedule.map(s=>(
            <div key={s.task} style={{display:"flex", alignItems:"center", gap:12, padding:"12px 0", borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
              <div style={{width:3, height:40, borderRadius:2, background:s.color, flexShrink:0}}></div>
              <div style={{flex:1}}>
                <div style={{fontSize:14, fontWeight:500}}>{s.task}</div>
                <div style={{fontSize:12, color:"var(--color-text-secondary)"}}>{s.detail}</div>
              </div>
              <div style={{fontSize:12, color:s.color, textAlign:"right"}}>{s.time}</div>
            </div>
          ))}
          <div style={{marginTop:16, background:"var(--color-background-secondary)", borderRadius:"var(--border-radius-md)", border:"0.5px solid var(--color-border-tertiary)", padding:"12px 14px"}}>
            <p style={{fontSize:13, margin:0, color:"var(--color-text-secondary)"}}>Miss a day? Resume tomorrow, not from scratch. The streak resets. The destination doesn't.</p>
          </div>
        </div>
      )}
      ...
      {view === "roadmap" && (
        <>
          ...
          <div style={{display:"flex", gap:6, overflowX:"auto", paddingBottom:6, marginBottom:"1rem", scrollbarWidth:"none"}}>
            {roadmap.map(p=>(
              <button key={p.id} onClick={()=>{setActivePhase(p.id); setExpandedWeek(null); setExpandedDay(null);}} style={{flexShrink:0, padding:"6px 12px", borderRadius:20, fontSize:12, cursor:"pointer", border:activePhase===p.id?`1.5px solid ${p.color}`:"0.5px solid var(--color-border-tertiary)", background:activePhase===p.id?"var(--color-background-secondary)":"transparent", color:activePhase===p.id?p.color:"var(--color-text-secondary)", fontWeight:activePhase===p.id?500:400}}>
                {p.label} · {p.title}
              </button>
            ))}
          </div>
          ...
          <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:"0.75rem"}}>
            <span style={{width:8, height:8, borderRadius:"50%", background:phase.color, flexShrink:0}}></span>
            <span style={{fontSize:13, color:"var(--color-text-secondary)"}}>{phase.weeks[0].days.split("–")[0].replace("Days ","Day ")} – {phase.weeks[phase.weeks.length-1].days.split("–")[1]}</span>
          </div>
          ...
          {phase.weeks.map(week=>{
            ...
            return (
              <div key={week.id} style={{marginBottom:8, border:`0.5px solid ${isOpen ? phase.color+"44" : "var(--color-border-tertiary)"}`, borderRadius:"var(--border-radius-lg)", overflow:"hidden", transition:"border-color 0.15s"}}>
                {/* Week header */}
                <button onClick={()=>toggleWeek(week.id)} style={{width:"100%", padding:"12px 14px", display:"flex", alignItems:"center", gap:10, background:isOpen?"var(--color-background-secondary)":"var(--color-background-primary)", border:"none", cursor:"pointer", textAlign:"left"}}>
                  <div style={{width:6, height:6, borderRadius:"50%", background:phase.color, flexShrink:0}}></div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:14, fontWeight:500, color:"var(--color-text-primary)"}}>{week.title}</div>
                    <div style={{fontSize:12, color:"var(--color-text-secondary)"}}>{week.range} · {week.days}</div>
                  </div>
                  <span style={{fontSize:16, color:phase.color, transform:isOpen?"rotate(90deg)":"rotate(0deg)", transition:"transform 0.15s", lineHeight:1}}>›</span>
                </button>
                ...
                {isOpen && (
                  <div style={{borderTop:"0.5px solid var(--color-border-tertiary)"}}>
                    {week.days_data.map((day, i)=>{
                      ...
                      const isPast = day.d < dayNum;
                      const isToday = day.d === dayNum;
                      return (
                        <div key={day.d} style={{borderBottom: i < week.days_data.length-1 ? "0.5px solid var(--color-border-tertiary)" : "none"}}>
                          <button onClick={()=>toggleDay(key)} style={{width:"100%", padding:"10px 14px", display:"flex", alignItems:"center", gap:10, background:isToday?"var(--color-background-info)":isDayOpen?"var(--color-background-secondary)":"transparent", border:"none", cursor:"pointer", textAlign:"left"}}>
                            {/* Day indicator */}
                            <div style={{width:26, height:26, borderRadius:"50%", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:500, background: isToday ? phase.color : isPast ? "var(--color-background-secondary)" : "transparent", border: isToday ? "none" : `0.5px solid ${isPast ? "var(--color-border-secondary)" : "var(--color-border-tertiary)"}`, color: isToday ? "#fff" : isPast ? "var(--color-text-secondary)" : "var(--color-text-tertiary)"}}>
                              {day.d}
                            </div>
                            <div style={{flex:1, minWidth:0}}>
                              <div style={{fontSize:13, fontWeight:isToday?500:400, color:isToday?"var(--color-text-info)":"var(--color-text-primary)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>{day.t}{isToday?" ← Today":""}</div>
                            </div>
                            <span style={{fontSize:13, color:"var(--color-text-tertiary)", transform:isDayOpen?"rotate(90deg)":"rotate(0deg)", transition:"transform 0.15s", flexShrink:0}}>›</span>
                          </button>
                          {isDayOpen && (
                            <div style={{padding:"0 14px 12px 50px", fontSize:13, color:"var(--color-text-secondary)", lineHeight:1.7}}>
                              {day.desc}
                            </div>
                          )}
                        </div>
      ...
      <div style={{marginTop:"1.25rem", display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
        {[
          {label:"Target by Day 365", val:"₹15–40L / yr", color:"#34D399"},
          {label:"Products to ship", val:"3 live apps", color:"#4A9EFF"},
          {label:"LLMs fine-tuned", val:"Mistral + Llama 3", color:"#A78BFA"},
          {label:"Career title", val:"ML / AI Engineer", color:"#FB923C"},
        ].map(o=>(
          <div key={o.label} style={{background:"var(--color-background-secondary)", borderRadius:"var(--border-radius-md)", border:"0.5px solid var(--color-border-tertiary)", padding:"10px 12px"}}>
            <div style={{fontSize:11, color:"var(--color-text-secondary)", marginBottom:2}}>{o.label}</div>
            <div style={{fontSize:14, fontWeight:500, color:o.color}}>{o.val}</div>
          </div>
        ))}
      </div>
    </div>
